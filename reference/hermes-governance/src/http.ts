...
export function createHermesGovernanceHttpServer(service: HermesGovernanceService, apiToken: string, actorSigningSecret?: string): Server {
  return createServer(async (req, res) => { try {
    ...
    if (req.method === 'POST' && memoryMatch?.[1]) { if (!trustedHuman(req, url.pathname, actor, actorSigningSecret)) return json(res, 403, { error: { code: 'INVALID_ACTOR_SIGNATURE' } }); return json(res, 200, await service.revokeMemory(decodeURIComponent(memoryMatch[1]), actor, 'human')); }
    ...
    if (id && match?.[2] === 'approve' && req.method === 'POST') { if (!trustedHuman(req, url.pathname, actor, actorSigningSecret)) return json(res, 403, { error: { code: 'INVALID_ACTOR_SIGNATURE' } }); return json(res, 200, await service.approve(id, actor, 'human')); }
    if (id && match?.[2] === 'reject' && req.method === 'POST') { if (!trustedHuman(req, url.pathname, actor, actorSigningSecret)) return json(res, 403, { error: { code: 'INVALID_ACTOR_SIGNATURE' } }); return json(res, 200, await service.reject(id, actor, 'human')); }
...
function trustedHuman(req: IncomingMessage,path:string,actor:string,secret?:string):boolean { if(!secret)return header(req,'x-actor-type')==='human'; const supplied=header(req,'x-actor-signature');const expected=createHmac('sha256',secret).update(`${req.method}:${path}:${actor}:human`).digest('hex');const a=Buffer.from(supplied),b=Buffer.from(expected);return a.length===b.length&&timingSafeEqual(a,b); }
